...
def test_on_val(val_dataloader, model, criterion):
    ...
    for input_tensor, target, metadata in val_dataloader:
        ...
        for idx in maxindex:
            if(torch.equal(maxindex[idx],target[idx])):
                n_correct += 1
...
def train(batch, model, criterion, optimizer):
    input_tensor, target,metadata  = batch
    input_tensor = Variable(input_tensor.cuda(async=True))
    target = Variable(target.cuda(async=True))
    optimizer.zero_grad()
    model_output = model(input_tensor)
...
def main():
    ...
    train_dataloader = DataLoader(train_dataset, args.batch_size, num_workers=4, sampler=GroupedSampler(train_dataset, rand=True),collate_fn=SortByWidthCollater_scriptid,
                                  pin_memory=True, drop_last=True)

    validation_dataloader = DataLoader(validation_dataset, args.batch_size, num_workers=0,sampler=GroupedSampler(validation_dataset, rand=False), collate_fn=SortByWidthCollater_scriptid,
    ...
    if args.load_from_snapshot is not None:
        model = CnnScriptIdModel.FromSavedWeights(args.load_from_snapshot)
    else:
        model = CnnScriptIdModel(
            num_in_channels=3,
