...
if args.dataset.upper() == "IAM":
    ...
    test_dataset = IAMDataset(args.datadir, "test", alphabet, args.line_height, line_img_transforms)
elif args.dataset.upper() == "MADCAT":
    test_dataset = MadcatDataset(args.datadir, "test", args.line_height, line_img_transforms)
else:
...
#    #logger.info("skipping MADCAT LM!!!!")
...
cer_running_avg = 0
wer_running_avg = 0
cer_running_avg_lm = 0
wer_running_avg_lm = 0
writer_id_running_accuracy = 0
n_samples = 0
...
for input_tensor, target, input_widths, target_widths, metadata in test_dataloader:
    ...
    prob_success = torch.nn.functional.softmax(conf_output, dim=2).cpu().data.numpy()[:,0,0]
